...
render_menu_ui :: proc(app: ^entities.App_State) {
	...
	// Continue (only if savegame exists) — entra directo al visualizador de la
	// campaña activa. El jugador retoma desde donde quedó.
	if savegame_exists {
		...
		if render_button(
			...
			// Forzar reload de campaign.bin desde disco
			app.campaign_loaded = false
...
render_map_browser :: proc(app: ^entities.App_State) {
	...
	// ── Right: preview o campaign editor según modo ──────────────────────────
	...
	if app.editor.campaign_editor.active {
		...
		// Campaña — toggle al editor de campaña dentro del mismo browser.
